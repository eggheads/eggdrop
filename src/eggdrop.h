...
#define BASELANG                                                               \
  "english" /* language which always gets loaded before                        \
...
#ifndef STATIC
#if !defined(MODULES_OK) ||                                                    \
    (!defined(MOD_USE_DL) && !defined(MOD_USE_SHL) &&                          \
     !defined(MOD_USE_DYLD) && !defined(MOD_USE_RLD) &&                        \
     !defined(MOD_USE_LOADER))
...
struct dcc_t {
  ...
  unsigned long status; /* A LOT of dcc types have status things; makes it more
                           availabe. */
  union {
...
/* Flags for dcc types. */
#define DCT_CHAT                                                               \
  0x00000001                     /* this dcc type receives botnet              \
                                  * chatter                          */
...
#define DCT_VALIDIDX                                                           \
  0x00000010                    /* valid idx for outputting to                 \
                                 * in tcl                           */
...
#define DCT_FILESEND                                                           \
  0x00000800                  /* a sending file transfer,                      \
                               * getting = !this                  */
...
#define STAT_FPRINT                                                            \
  0x00002                    /* fingerprint auth (ignore password &            \
                              * allow access)                        */
...
#define STAT_LINKING                                                           \
  0x00100                       /* the bot is currently going through          \
                                 * the linking stage                     */
...
#define SOCK_PASS                                                              \
  0x0100                    /* passed on; only notify in case                  \
                             * of traffic                           */
