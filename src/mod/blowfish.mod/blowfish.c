...
static char *encrypt_string_cbc(char *key, char *str) {
  ...
  *p = 0;
  ...
  for (i = 0; i < slen - 2; i += 3) {
    ...
    *p++ =
        cbcbase64[((s[i + 1] & 0x0f) << 2) | (((unsigned char)s[i + 2]) >> 6)];
...
static tcl_cmds mytcls[] = {{"encrypt", tcl_encrypt},
                            {"decrypt", tcl_decrypt},
                            {"encpass", tcl_encpass},
                            {NULL, NULL}};

static tcl_strings my_tcl_strings[] = {{"blowfish-use-mode", bf_mode, 3, 0},
                                       {NULL, NULL, 0, 0}};
...
static Function blowfish_table[] = {
    ...
    (Function)blowfish_expmem, (Function)blowfish_report,
    /* 4 - 7 */
    (Function)encrypt_string, (Function)decrypt_string,
};
