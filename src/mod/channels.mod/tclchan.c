...
static int tcl_stick STDVAR {
  ...
  if (!ok && u_setsticky_ban(NULL, argv[1], !strncmp(argv[0], "un", 2) ? 0 : 1))
    ok = 1;
  if (ok)
    Tcl_AppendResult(irp, "1", NULL);
  else
...
static int tcl_stickinvite STDVAR {
  ...
  if (!ok &&
      u_setsticky_invite(NULL, argv[1], !strncmp(argv[0], "un", 2) ? 0 : 1))
    ok = 1;
  if (ok)
    Tcl_AppendResult(irp, "1", NULL);
  else
...
static int tcl_stickexempt STDVAR {
  ...
  if (!ok &&
      u_setsticky_exempt(NULL, argv[1], !strncmp(argv[0], "un", 2) ? 0 : 1))
    ok = 1;
  if (ok)
    Tcl_AppendResult(irp, "1", NULL);
  else
...
static int tcl_channel_getlist(Tcl_Interp *irp, struct chanset_t *chan) {
  ...
  APPEND_KEYVAL("enforcebans", channel_enforcebans(chan) ? "1" : "0");
  APPEND_KEYVAL("dynamicbans", channel_dynamicbans(chan) ? "1" : "0");
  APPEND_KEYVAL("userbans", channel_nouserbans(chan) ? "1" : "0");
  APPEND_KEYVAL("autoop", channel_autoop(chan) ? "1" : "0");
  APPEND_KEYVAL("autohalfop", channel_autohalfop(chan) ? "1" : "0");
  APPEND_KEYVAL("bitch", channel_bitch(chan) ? "1" : "0");
  APPEND_KEYVAL("greet", channel_greet(chan) ? "1" : "0");
  APPEND_KEYVAL("protectops", channel_protectops(chan) ? "1" : "0");
  APPEND_KEYVAL("protecthalfops", channel_protecthalfops(chan) ? "1" : "0");
  APPEND_KEYVAL("protectfriends", channel_protectfriends(chan) ? "1" : "0");
  APPEND_KEYVAL("dontkickops", channel_dontkickops(chan) ? "1" : "0");
  APPEND_KEYVAL("inactive", channel_inactive(chan) ? "1" : "0");
  APPEND_KEYVAL("statuslog", channel_logstatus(chan) ? "1" : "0");
  APPEND_KEYVAL("revenge", channel_revenge(chan) ? "1" : "0");
  APPEND_KEYVAL("revengebot", channel_revengebot(chan) ? "1" : "0");
  APPEND_KEYVAL("secret", channel_secret(chan) ? "1" : "0");
  APPEND_KEYVAL("shared", channel_shared(chan) ? "1" : "0");
  APPEND_KEYVAL("autovoice", channel_autovoice(chan) ? "1" : "0");
  APPEND_KEYVAL("cycle", channel_cycle(chan) ? "1" : "0");
  APPEND_KEYVAL("seen", channel_seen(chan) ? "1" : "0");
  APPEND_KEYVAL("nodesynch", channel_nodesynch(chan) ? "1" : "0");
  APPEND_KEYVAL("static", channel_static(chan) ? "1" : "0");
  APPEND_KEYVAL("dynamicexempts", channel_dynamicexempts(chan) ? "1" : "0");
  APPEND_KEYVAL("userexempts", channel_nouserexempts(chan) ? "1" : "0");
  APPEND_KEYVAL("dynamicinvites", channel_dynamicinvites(chan) ? "1" : "0");
  APPEND_KEYVAL("userinvites", channel_nouserinvites(chan) ? "1" : "0");
...
static int tcl_channel_get(Tcl_Interp *irp, struct chanset_t *chan,
  ...
  if (!strcmp(setting, "chanmode"))
    ...
    CHKFLAG_POS(CHAN_ENFORCEBANS, "enforcebans", chan->status)
  else if
    CHKFLAG_POS(CHAN_DYNAMICBANS, "dynamicbans", chan->status)
  else if
    CHKFLAG_NEG(CHAN_NOUSERBANS, "userbans", chan->status)
  else if
    CHKFLAG_POS(CHAN_OPONJOIN, "autoop", chan->status)
  else if
    CHKFLAG_POS(CHAN_AUTOHALFOP, "autohalfop", chan->status)
  else if
    CHKFLAG_POS(CHAN_BITCH, "bitch", chan->status)
  else if
    CHKFLAG_POS(CHAN_GREET, "greet", chan->status)
  else if
    CHKFLAG_POS(CHAN_PROTECTOPS, "protectops", chan->status)
  else if
    CHKFLAG_POS(CHAN_PROTECTHALFOPS, "protecthalfops", chan->status)
  else if
    CHKFLAG_POS(CHAN_PROTECTFRIENDS, "protectfriends", chan->status)
  else if
    CHKFLAG_POS(CHAN_DONTKICKOPS, "dontkickops", chan->status)
  else if
    CHKFLAG_POS(CHAN_INACTIVE, "inactive", chan->status)
  else if
    CHKFLAG_POS(CHAN_LOGSTATUS, "statuslog", chan->status)
  else if
    CHKFLAG_POS(CHAN_REVENGE, "revenge", chan->status)
  else if
    CHKFLAG_POS(CHAN_REVENGEBOT, "revengebot", chan->status)
  else if
    CHKFLAG_POS(CHAN_SECRET, "secret", chan->status)
  else if
    CHKFLAG_POS(CHAN_SHARED, "shared", chan->status)
  else if
    CHKFLAG_POS(CHAN_AUTOVOICE, "autovoice", chan->status)
  else if
    CHKFLAG_POS(CHAN_CYCLE, "cycle", chan->status)
  else if
    CHKFLAG_POS(CHAN_SEEN, "seen", chan->status)
  else if
    CHKFLAG_POS(CHAN_NODESYNCH, "nodesynch", chan->status)
  else if
    CHKFLAG_POS(CHAN_STATIC, "static", chan->status)
  else if
    CHKFLAG_POS(CHAN_DYNAMICEXEMPTS, "dynamicexempts", chan->ircnet_status)
  else if
    CHKFLAG_NEG(CHAN_NOUSEREXEMPTS, "userexempts", chan->ircnet_status)
  else if
    CHKFLAG_POS(CHAN_DYNAMICINVITES, "dynamicinvites", chan->ircnet_status)
  else if
    CHKFLAG_NEG(CHAN_NOUSERINVITES, "userinvites", chan->ircnet_status)
  else {
        ...
        *p++ = 0;
        ...
        if ((*item[i]) && !strtol(item[i], &endptr, 10) && !(*endptr)) {
          ...
          *ptime = 0;
          ...
          if (irp)
            Tcl_AppendResult(
                irp, "flood value must be in X:Y format: ", item[i], NULL);
          return TCL_ERROR;
        }
      }
...
static int tcl_chansettype STDVAR {
  ...
  /* String values first */
  if (!strcmp(argv[1], "chanmode") || !strcmp(argv[1], "need-op") ||
      !strcmp(argv[1], "need-invite") || !strcmp(argv[1], "need-key") ||
      !strcmp(argv[1], "need-unban") || !strcmp(argv[1], "need-limit")) {
    Tcl_AppendResult(irp, "str", NULL);
    /* Couplets */
  } else if (!strcmp(argv[1], "flood-chan") || !strcmp(argv[1], "flood-ctcp") ||
             !strcmp(argv[1], "flood-join") || !strcmp(argv[1], "flood-kick") ||
             !strcmp(argv[1], "flood-deop") || !strcmp(argv[1], "flood-nick") ||
             !strcmp(argv[1], "flood-nick") || !strcmp(argv[1], "aop-delay")) {
    Tcl_AppendResult(irp, "pair", NULL);
    /* Integers now */
  } else if (!strcmp(argv[1], "idle-kick") ||
             !strcmp(argv[1], "stopnethack-mode") ||
             !strcmp(argv[1], "revenge-mode") || !strcmp(argv[1], "ban-type") ||
             !strcmp(argv[1], "ban-time") || !strcmp(argv[1], "exempt-time") ||
             !strcmp(argv[1], "invite-time")) {
    Tcl_AppendResult(irp, "int", NULL);
    /* Last, but not least - flags */
  } else if (!strcmp(argv[1], "enforcebans") ||
             !strcmp(argv[1], "dynamicbans") || !strcmp(argv[1], "userbans") ||
             !strcmp(argv[1], "autoop") || !strcmp(argv[1], "autohalfop") ||
             !strcmp(argv[1], "bitch") || !strcmp(argv[1], "greet") ||
             !strcmp(argv[1], "protectops") ||
             !strcmp(argv[1], "protecthalfops") ||
             !strcmp(argv[1], "protectfriends") ||
             !strcmp(argv[1], "dontkickops") || !strcmp(argv[1], "inactive") ||
             !strcmp(argv[1], "statuslog") || !strcmp(argv[1], "revenge") ||
             !strcmp(argv[1], "revengebot") || !strcmp(argv[1], "secret") ||
             !strcmp(argv[1], "shared") || !strcmp(argv[1], "autovoice") ||
             !strcmp(argv[1], "cycle") || !strcmp(argv[1], "seen") ||
             !strcmp(argv[1], "nodesynch") || !strcmp(argv[1], "static") ||
...
static tcl_cmds channels_cmds[] = {{"killban", tcl_killban},
