  ...
  for (chan = chanset; chan; chan = chan->next) {
    if ((m = ismember(chan, nick))) {
      if (rfc_casecmp(m->account, account)) {
        ...
        if (m->account[0]) {
          if (!strcmp(account, "*")) {
            putlog(LOG_MODE, chan->dname, "%s!%s has logged out of their account", nick, m->userhost);
          } else {
            putlog(LOG_MODE, chan->dname, "%s!%s logged in to their account %s", nick, m->userhost, account);
          }
  ...
  for (chan = chanset; chan; chan = chan->next) {
    ...
    if (m) {
      ...
      if (strlen(msg)) {
        m->flags |= IRCAWAY;
        fixcolon(msg);
        putlog(LOG_MODE, chan->dname, "%s is now away: %s", from, msg);
      } else {
        m->flags &= ~IRCAWAY;
        putlog(LOG_MODE, chan->dname, "%s has returned from away status", from);
      }
    }
  }
  return 0;
