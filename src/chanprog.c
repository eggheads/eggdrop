...
extern int backgrd, term_z, con_chan, cache_hit, cache_miss, firewallport,
    default_flags, max_logs, conmask, protect_readonly, make_userfile, noshare,
    ignore_time, max_socks;
#ifdef TLS
  ...
  dprintf(idx, "%s %s (%s %s)\n", MISC_TCLVERSION,
          ((interp) && (Tcl_Eval(interp, "info patchlevel") == TCL_OK))
              ...
              : (Tcl_Eval(interp, "info tclversion") == TCL_OK)
                    ? tcl_resultstring()
                    : "*unknown*",
          MISC_TCLHVERSION, TCL_PATCH_LEVEL);
...
/* Add a timer
 */
unsigned long add_timer(tcl_timer_t **stack, int elapse, int count, char *cmd,
  ...
  do {
    ...
    if (*ptr && !egg_isspace(*ptr) && *ptr != ',') {
      ...
      for (n = name; *n && *s && s < ptr &&
                     tolower((unsigned)*n) == tolower((unsigned)*s);
           n++, s++)
        ;
...
void add_hq_user() {
  if (!backgrd && term_z > 0 && userlist) {
    /* HACK: Workaround using dcc[].nick not to pass literal "-HQ" as a
     * non-const arg */
