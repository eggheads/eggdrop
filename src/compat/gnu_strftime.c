...
static struct tm *my_strftime_gmtime_r(t, tp) const time_t *t;
struct tm *tp;
{
...
static struct tm *my_strftime_localtime_r(t, tp) const time_t *t;
struct tm *tp;
{
...
#define cpy(n, s)                                                              \
  add((n), if (to_lowcase) memcpy_lowcase(p, (s), _n);                         \
      else if (to_uppcase) memcpy_uppcase(p, (s), _n);                         \
      else memcpy((PTR)p, (PTR)(s), _n))
...
static char *memcpy_lowcase(dest, src, len) char *dest;
const char *src;
size_t len;
{
  while (len-- > 0)
...
static char *memcpy_uppcase(dest, src, len) char *dest;
const char *src;
size_t len;
{
  while (len-- > 0)
...
static int tm_diff(a, b) const struct tm *a;
const struct tm *b;
{
...
__inline__
#endif
    static int iso_week_days(yday, wday) int yday;
int wday;
{
...
size_t my_strftime(s, maxsize, format, tp ut_argument) char *s;
size_t maxsize;
const char *format;
const struct tm *tp;
ut_argument_spec {
...
/* POSIX.1 8.1.1 requires that whenever strftime() is called, the
  ...
  for (f = format; *f != '\0'; ++f) {
    ...
    const char *subfmt;
    char *bufp;
    char buf[1 + (sizeof(int) < sizeof(time_t) ? INT_STRLEN_BOUND(time_t)
    ...
    case 'y':
      if (modifier == 'E') {
        ...
        if (era) {
          ...
          DO_NUMBER(1,
                    (era->offset + (era->direction == '-' ? -delta : delta)));
        }
#else
#if HAVE_STRFTIME
        goto underlying_strftime;
...
size_t emacs_strftime(s, maxsize, format, tp) char *s;
size_t maxsize;
const char *format;
const struct tm *tp;
